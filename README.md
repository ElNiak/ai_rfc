...
```bash
python -m experiment workspace prepare mark --panther-repo <PANTHER>
...
entries and tags on disk — so each session picks up where the artifacts say the
last one stopped rather than where a counter thought it was. `--budget` caps the
**run**, not the session: each session is handed what the run has left, so
sixty-nine clusters cannot spend sixty-nine times the flag. A cluster that will
not finish in `ATTEMPTS_PER_CLUSTER` halts the sweep rather than being skipped,
because later prose builds on earlier prose and a draft with a hole in it is
worse than a short one.

Sessions resume inside a run; **a run itself is never relaunched**, because
`runner.launch` writes `status.json` exactly once and refuses a run that already
has one. What survives a stopped run is the workspace, so a sweep is continued by
chaining a *new* campaign onto it:

```bash
python -m experiment workspace reseal <run-dir>/workspace --as mark-cont1
python -m experiment campaign init --id mark-full-2 --baseline mark-cont1 \
    --arms A --repeats 1 --session-mode per-cluster \
    --budget 60 --timeout 7200 --panther-repo <PANTHER>
python -m experiment run <campaign-dir>
```

`reseal` copies the workspace, re-reads the draft HEAD the run advanced to and
seals the copy, leaving the run directory untouched so its audit still reads the
evidence it recorded. Chaining modest slices this way is what makes a long sweep
survivable: a budget or wall-clock stop costs one slice, not the whole run.
