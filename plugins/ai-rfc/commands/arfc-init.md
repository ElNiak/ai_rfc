...
Initialize the reconstruction workspace at `$ARFC_WORKSPACE` for the target
repository given as `$ARGUMENTS` (a forge URL). `PANTHER_REPO` and
`ARFC_WORKSPACE` must be set; stop with a clear message if either is
...
6. **Draft scaffold**: clone `https://github.com/ElNiak/auto-i-d-template`
   into `$ARFC_WORKSPACE/draft`, remove its `.git`, `git init -b main`,
