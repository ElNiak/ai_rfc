...
**One measured asymmetry (CLI-3 D16).** Folding the parity CLI into `ai-rfc` left `ai_rfc_status` without a verb of its own, because `ai-rfc status` already names the operator's ledger. Arm A therefore reaches twenty operations as tools and arm B reaches nineteen as verbs — a change to the *arm surfaces themselves*, not to the documentation of them, and the first time the two arms have not been the same size. It is not a capability delta of the kind the paragraph above calls a stop-ship: the status read is a composite over four artifacts both arms can already open, and arm B additionally has `ai-rfc status`. What it costs arm B is calls, so every per-call measure is read against it, and the twentieth twin drives the tool against `queries.status` rather than against a verb. Recorded here rather than corrected, on the same rule as every other asymmetry in `docs/parity.md`.

**A second measured asymmetry (2026-09-22, `0e0062f`).** The `ai_rfc_corpus_query` tool's description now lists the corpus index's tables and columns, derived from the DDL by sqlite at import, after a Phase 5 arm-A run guessed three columns that do not exist. Arms B and C reach the same index through the verb and `sqlite3` with only the prompt slot's single example as a hint, so this gap costs them accuracy, not calls — an arm that guesses column names pays in failed queries. It is disclosed here rather than corrected because the correction is a prompt change: deriving the `{{corpus_query}}` slot from the same summary regenerates every arm's rendered prompt and re-freezes the instrument, which belongs to the main-campaign map (owed #62b), not to a fix wave. Until it lands, any per-query accuracy measure is read knowing arm A had the schema and arms B and C did not.
