...
def test_classify_maps_tools_to_surfaces():
    assert classify("mcp__ai_rfc__ai_rfc_checkpoint", {"cluster_id": "c"}, WS) == (
        "mcp",
        "ai_rfc_checkpoint",
        "",
    )
...
def test_audit_events_flags_an_executed_out_of_arm_call():
    ...
    assert audit["executed_out_of_arm"][0]["surface"] == "bash:ai_rfc"
...
def test_a_denial_that_never_reached_the_result_event_falls_back_to_its_text():
    events = parse_stream(
        '{"type":"assistant","message":{"id":"m1","content":[{"type":"tool_use","id":"t1","name":"mcp__ai_rfc__ai_rfc_status","input":{}}],"usage":{"input_tokens":1,"output_tokens":1}}}\n'
...
def test_audit_over_fake_runs_counts_bypasses_and_errors(campaign, write_scenario):
    ...
    assert a["bypass_attempts"]["by_surface"] == {"bash:ai_rfc": 1}
    assert a["errors"] == {
        "class1": 1,
        "class2": 0,
