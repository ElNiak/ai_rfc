...
#: stands differs. Arm A has no shell to ask git with and calls
...
SLOT_TABLES: dict[str, dict[str, str]] = {
    ...
    # Arm B reaches the one command tree through the campaign's own bin/ai-rfc
    # shim, so every slot here spells a grouped verb the root parser dispatches
    # -- `ai-rfc claim upsert` -- and not the retired hyphenated leaf form on
    # the retired console script. The Bash family that admits these is
    # declared once, in `arms.py`.
    "B": {
        "guidance": _EXPERIMENT_GUIDANCE,
        "preamble": (
            "This session has no MCP server: drive the workspace with the `ai-rfc` "
            "command through Bash, exactly as written below."
        ),
        "runtime": "`ai-rfc` is on `PATH` and reads `AI_RFC_WORKSPACE`",
        "cluster_next": (
            "`ai-rfc cluster next` (prints the lowest-ordinal in-window cluster "
        ...
        "draft_lint": "`ai-rfc draft lint`",
        "checkpoint_consolidation": (
            "`ai-rfc checkpoint <id> --consolidation NN --base checkpoints/<id>`"
        ),
        "revision_record_consolidation": (
            "`ai-rfc revision record draft-<name>-NN --cluster <id> "
            "--kind consolidation --checkpoint consolidations/NN "
            '--no-normative --note "…"`'
        ),
        "cluster_get": (
            "`ai-rfc cluster get <id> --patch` (add `--patch-offset N "
        ...
        "record_status": (
            "`ai-rfc claim check` to see stored vs supported, then "
            "`ai-rfc claim record-status`"
        ),
        "gate": "`ai-rfc gate --strict`",
        "checkpoint": "`ai-rfc checkpoint <id>`",
        "revision_record": (
            "`ai-rfc revision record draft-<name>-NN --cluster <id> "
            '--normative|--no-normative --note "…"`'
        ),
        "draft_commit": '`ai-rfc draft commit -m "<message>"`',
        "draft_build": (
            "`ai-rfc draft build` — exit 0 and no findings is the bar before "
        ...
        "revision_tag": (
            '`ai-rfc revision tag draft-<name>-NN -m "<message>"` — it runs the '
        ...
        "citation_gate": "`ai-rfc citation-gate --strict`",
        "question_draft": (
            '`ai-rfc question draft "<question quoting the claim text verbatim>" '
            "--claim <id>`"
        ),
    },
    "A": {
...
def task_profile(name: str) -> TaskProfile:
    ...
    Raises:
        DriverError: If no profile carries that name.
    """
...
def _refuse_slots(text: str, source: str) -> None:
    ...
    Raises:
        DriverError: If ``text`` holds anything matching :data:`SLOT_RE`.
    """
    found = sorted(set(SLOT_RE.findall(text)))
    if found:
        raise DriverError(f"{source} leaves slots unrendered: {found}")
...
def _render_template(name: str, arm: str, template: str | None = None) -> str:
    ...
    Raises:
        DriverError: If ``arm`` has no table, a slot stays unfilled, or a
    ...
    if missing:
        raise DriverError(f"table {arm!r} leaves slots unfilled: {missing}")
...
def render_loop(arm: str, template: str | None = None) -> str:
    ...
    Raises:
        DriverError: If ``arm`` has no table or a slot stays unfilled.
    """
...
def arm_prompt(
    ...
    *,
    ...
    Raises:
        DriverError: If no profile carries that name, the profile does not
...
def consolidation_prompt(arm: str, plugin_root: Path) -> str:
    ...
    Raises:
        DriverError: As :func:`arm_prompt`.
    """
