...
Every command is also reachable as `python -m ai_rfc <verb>`, the same
dispatcher the `ai-rfc` script runs; the experiment's raw arm invokes that
form.
...
```bash
ai-rfc history path/to/clone --out corpus/
# ... mining happens here, outside this framework ...
ai-rfc check manifest.yaml --out out/ --repo path/to/clone
ai-rfc check manifest.yaml --out out/ --repo path/to/clone --strict
```
...
| stderr `report` (consolidated, not a copy) | `lifecycle/common.py` — every lifecycle verb imports this one rather than keeping its own, and so does `draft/cli.py`, which had the eighth `_report` until a `cluster_id` out of `revisions.yaml` was shown forging a `note: gate clean` line through it. Not tracked by `TRACKED_HELPERS`: `` `_report` `` and `` `report` `` are substrings of one another, so the table's row lookup cannot tell the two apart. Listed here so the `_report` row above is not read as the whole picture. |
